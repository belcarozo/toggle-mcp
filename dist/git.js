...
function parseSubject(subject) {
    ...
    if (checkout)
        return { kind: "checkout", description: null, toBranch: checkout[2] };
    ...
    if (subject.startsWith("pull"))
        return { kind: "pull", description: null };
    if (subject.startsWith("reset"))
        return { kind: "reset", description: null };
    return null;
}
/** A checkout target that isn't a real branch (SHA, detached HEAD, tag ref, etc). */
export function looksLikeBranch(ref, baseBranches = DEFAULT_BASE_BRANCHES) {
    if (baseBranches.includes(ref))
        return false;
 ...
 */
export function parseReflog(raw, options) {
    ...
    for (const line of lines) {
        ...
        if (!currentBranch)
            continue; // no known branch to attribute this to
        ...
        if (timestamp < windowStart || timestamp > windowEnd)
            continue; // lookback was for branch context only
        events.push({
            timestamp,
            repo: repoName,
 ...
 */
export function readReflogEvents(repo, sinceIso, untilIso, zone, baseBranches = DEFAULT_BASE_BRANCHES) {
    ...
    let raw;
    try {
        raw = execFileSync("git", ["-C", repo, "reflog", "--date=iso", `--since=${lookbackStart} 00:00:00`, `--until=${untilIso} 23:59:59`], { encoding: "utf8", maxBuffer: 1024 * 1024 * 16 });
    }
    catch (err) {
        throw new Error(`toggl-mcp: failed to read git reflog for ${repo}: ${err.message}`);
    }
    return parseReflog(raw, { sinceIso, untilIso, zone, repoName: basename(repo), baseBranches });
}
export function readAllRepos(repos, sinceIso, untilIso, zone, baseBranches = DEFAULT_BASE_BRANCHES) {
