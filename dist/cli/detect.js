...
export function sampleBranches(repo, count = 20) {
    try {
        const raw = execFileSync("git", ["-C", repo, "for-each-ref", "--sort=-committerdate", "--format=%(refname:short)", "refs/heads"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
...
export function previewTicketMatches(pattern, branches) {
    let regex;
    try {
