...
import { sanitizeSignalText } from "./context.js";
 ...
 */
export function parseSessionFile(lines, options) {
    ...
    for (const line of lines) {
        ...
        if (record.type === "ai-title") {
            if (record.aiTitle)
                title = record.aiTitle; // last one in the file wins
            continue;
        }
        if (!record.type || !MESSAGE_TYPES.has(record.type))
            continue;
        if (record.isSidechain || record.isMeta)
            continue;
        if (!record.timestamp || !record.cwd)
            continue;
    ...
    for (const record of inWindow) {
        ...
        if (last && (last.branch !== record.branch || last.cwd !== record.cwd))
            flushRun();
        run.push(record);
    }
    flushRun();
 ...
 */
export function readSessions(repos, sinceIso, untilIso, zone, ticketPattern = DEFAULT_TICKET_PATTERN, root = TRANSCRIPTS_ROOT) {
    ...
    for (const dir of projectDirs) {
        ...
        for (const file of files) {
            ...
            try {
                if (statSync(path).mtimeMs < cutoffMillis)
                    continue;
