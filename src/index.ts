...
function resolveRange(
  config: Config,
  input: { week?: "this" | "last"; startDate?: string; endDate?: string },
): { weekStart: string; weekEnd: string } {
  if (input.week) return resolveWeek(input.week, config.timezone);
...
server.registerTool(
  "toggl_whoami",
  {
    title: "Toggl whoami",
    description:
      `Resolve the Toggl account, default workspace, and the "${DEFAULT_CONFIG_PATH}" project name to their IDs. ` +
...
server.registerTool(
  "apply_week",
  {
    ...
    description:
      "Write the exact proposal returned by plan_week to Toggl. Re-checks each day for existing entries immediately " +
