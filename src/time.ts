...
export function weekdayOf(dt: DateTime): Weekday {
  const entry = Object.entries(WEEKDAY_TO_LUXON).find(([, n]) => n === dt.weekday);
  if (!entry) throw new Error(`unreachable: luxon weekday ${dt.weekday}`);
  return entry[0] as Weekday;
}

/** Build a DateTime for `date` (YYYY-MM-DD) at local "HH:mm" `time`, in `zone`. */
export function atLocalTime(date: string, time: string, zone: string): DateTime {
...
export function mergeIntervals(intervals: Interval[]): Interval[] {
  ...
  for (const current of sorted.slice(1)) {
    ...
    if (current.start.toMillis() <= last.end.toMillis()) {
      if (current.end.toMillis() > last.end.toMillis()) last.end = current.end;
    } else {
      merged.push({ ...current });
    }
  }
...
export function clipToWindow(interval: Interval, window: Interval): Interval | null {
  ...
  if (start.toMillis() >= end.toMillis()) return null;
  return { start, end };
}
...
export function subtractIntervals(base: Interval, subtract: Interval[]): Interval[] {
  ...
  for (const block of clipped) {
    ...
    if (block.end.toMillis() > cursor.toMillis()) cursor = block.end;
  }
...
export function roundInterval(interval: Interval, minutes: number): Interval {
  ...
  if (end.toMillis() <= start.toMillis()) end = start.plus({ minutes });
  return { start, end };
}
...
export interface WeekRange {
  /** YYYY-MM-DD, Monday */
  weekStart: string;
  /** YYYY-MM-DD, last day to plan (today for "this", Friday for "last") */
  weekEnd: string;
}
 ...
 */
export function resolveWeek(which: "this" | "last", zone: string, now: DateTime = DateTime.now().setZone(zone)): WeekRange {
...
export function eachDate(weekStart: string, weekEnd: string, zone: string): string[] {
  const dates: string[] = [];
...
export function toTogglUtc(dt: DateTime): string {
