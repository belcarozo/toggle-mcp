...
export interface RunDoctorOptions {
  path?: string;
  /** Also verify live Toggl connectivity. Costs API calls against the 30/hour /me budget - opt-in only. */
  online?: boolean;
}
...
export async function runDoctor(options: RunDoctorOptions = {}): Promise<DoctorReport> {
  ...
  checks.push(
    DateTime.now().setZone(config.timezone).isValid
  ...
  if (options.online && process.env.TOGGL_API_TOKEN) {
    try {
      ...
      checks.push({ name: "Toggl connectivity", status: "fail", message: (err as Error).message });
    }
  }
