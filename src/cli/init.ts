...
import { STARTER_CONFIG, answersToConfig, buildFieldSpecs, defaultAnswer, parseRepoList } from "./questions.js";

function renderRegistrationSnippet(entryPath: string): string {
...
async function collectRepos(prompter: Prompter, base: Config): Promise<string[]> {
  const spec = buildFieldSpecs().find((s) => s.id === "repos")!;
  return askUntilValid(prompter, `${spec.prompt} [${defaultAnswer(spec, base)}]`, (raw) => {
    const repos = raw.trim() === "" ? (base.repos as string[]) : (spec.parse(raw) as string[]);
...
async function collectTicketPattern(prompter: Prompter, base: Config, repos: string[]): Promise<string> {
  ...
  if (branches.length > 0) {
    ...
    for (const p of preview.slice(0, 5)) console.log(`  ${p.branch} -> ${p.match ?? "(no match)"}`);
  } else {
    console.log("\n(no branches found to preview against - pattern will be validated for syntax only)");
  }

  for (;;) {
    const pattern = await askUntilValid(prompter, `${spec.prompt} [${defaultAnswer(spec, base)}]`, (raw) =>
      raw.trim() === "" ? base.ticketPattern : (spec.parse(raw) as string),
    );
    if (branches.length === 0 || pattern === base.ticketPattern) return pattern;
    const preview = previewTicketMatches(pattern, branches);
    const matched = preview.filter((p) => p.match !== null).length;
    console.log(`"${pattern}" matches ${matched}/${branches.length} sampled branches.`);
    for (const p of preview.slice(0, 5)) console.log(`  ${p.branch} -> ${p.match ?? "(no match)"}`);
    if (await prompter.confirm("Use this pattern?", true)) return pattern;
  }
}
...
export async function runInit(prompter: Prompter, options: RunInitOptions = {}): Promise<number> {
  const path = options.path ?? DEFAULT_CONFIG_PATH;

  let base: Config = { ...STARTER_CONFIG, timezone: detectTimezone() ?? STARTER_CONFIG.timezone };
  ...
  for (const spec of buildFieldSpecs()) {
    if (spec.id === "repos" || spec.id === "ticketPattern") continue; // already collected above
